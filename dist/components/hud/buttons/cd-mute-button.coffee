do ()->
	button = document.createElement "cd-mute-button"
	button.innerHTML = '
		<svg height="1.5em" version="1.1" width="1.5em" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
	...
	updateText = ()->
		call = if muted then "add" else "remove"
		button.firstChild.classList[call]("muted")
	
	fireEvent = ()->
		eventString = if muted then "disableAudio" else "enableAudio"
		window.dispatchEvent(new CustomEvent(eventString))
